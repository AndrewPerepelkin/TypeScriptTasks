...
const normalizeData = (unnormalizedData: IUnnormalizedData[]): INormalizeData => {
  const byId: IById = {}
  const allIds: string[] = []
  unnormalizedData.forEach((item: IUnnormalizedData) => {
    byId[item.id] = { ...item }
    allIds.push(item.id)
  })
  return {
